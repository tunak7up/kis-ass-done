...
//       const isPercentage = td.total_formula && td.total_formula.includes("* 100");
...
class KpiService {
  ...
  static async getKpiStructure(routeId, year, month) {
    ...
    const totalDetails = await TotalDetail.findAll({
      ...
      attributes: ['totalDetail_id', 'totalDetail_name', 'total_formula', 'day_formula'],
      order: [
        [{ model: Kpi, as: 'Kpi' }, { model: Content, as: 'Content' }, 'content_id', 'ASC'],
        [{ model: Kpi, as: 'Kpi' }, 'kpi_id', 'ASC'],
        ['totalDetail_id', 'ASC']
      ]
    });
    ...
    const results = await Promise.all(
      totalDetails.map(async (td) => {
        ...
        // ✅ Kiểm tra xem công thức có chứa * 100 không
        const isPercentage = td.total_formula && 
          (td.total_formula.includes('* 100') || 
           td.total_formula.includes('*100'));
        ...
        return {
          ...
          kpi_name: td.Kpi.kpi_name,
          totalDetail_name: td.totalDetail_name,
          // ✅ Thêm % nếu là percentage
          luyKe: isPercentage 
            ? `${typeof luyKe === 'number' ? luyKe.toFixed(2) : luyKe}%`
            : luyKe,
          content_id: td.Kpi.Content.content_id,
          kpi_id: td.Kpi.kpi_id,
          isPercentage: isPercentage // ✅ Thêm flag để frontend biết
        };
      })
    );
  ...
  static async calculateLuyKe(totalDetailId, formula, routeId, year, month, allTotalDetails) {
    ...
    try {
      ...
      if (formula.toLowerCase().includes('sum') || formula.toLowerCase().includes('tong')) {
        ...
        const sum = details.reduce((sum, d) => sum + (parseFloat(d.detail_value) || 0), 0);
        return sum;
      }
      ...
      if (rowRefMatch || tdRefMatch) {
        ...
        if (rowRefMatch) {
          for (const ref of rowRefMatch) {
            ...
            if (targetIndex >= 0 && targetIndex < allTotalDetails.length) {
              ...
              // ✅ Xử lý nếu targetValue là string có %
              const numericValue = typeof targetValue === 'string' 
                ? parseFloat(targetValue.replace('%', ''))
                : targetValue;
              calculatedFormula = calculatedFormula.replace(ref, numericValue || 0);
            } else {
              calculatedFormula = calculatedFormula.replace(ref, 0);
            }
          }
        }
        ...
        try {
          ...
          // ✅ Kiểm tra chia cho 0
          if (calculatedFormula.includes('/0') || calculatedFormula.match(/\/0[^\d]/)) {
            console.warn(`Division by zero in formula: ${formula}`);
            return 0;
          }
          ...
          if (!isFinite(result) || isNaN(result)) {
            console.warn(`Invalid result for formula: ${formula} = ${result}`);
            return 0;
          }
      ...
      console.error('Error calculating luy ke:', {
        totalDetailId,
        formula,
        error: error.message
      });
