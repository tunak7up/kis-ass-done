...
export default function CustomizedTables({
  ...
  useEffect(() => {
    if (selectedRoute && selectedYear && selectedMonth) {
      ...
      fetch(
        `${API_BASE}/kpi-structure?routeId=${selectedRoute}&year=${selectedYear}&month=${selectedMonth}`
      )
        .then((res) => {
  ...
  const processedRows = useMemo(() => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const isFirstInContent =
        i === 0 || data[i].content_name !== data[i - 1].content_name;
      ...
      if (isFirstInKpi) {
        ...
        for (let j = i + 1; j < data.length; j++) {
          if (
            data[j].content_name === data[i].content_name &&
  ...
  if (!selectedRoute || !selectedYear || !selectedMonth) {
    return (
      <div style={{ padding: "20px", textAlign: "center" }}>
        Vui lòng chọn Route, Năm và Tháng
      </div>
    );
  }
    ...
    <div className="customized-table">
      <TableContainer>
        <Table sx={{ minWidth: "50%" }} aria-label="customized table">
          <TableHead
            className="customizedTableHead"
            style={{ height: "113.4px" }}
          >
            <TableRow>
              ...
              <StyledTableRow key={row.totalDetail_id || index}>
                ...
                <StyledTableCell align="right">
                  {typeof row.luyKe === "number"
                    ? row.luyKe.toLocaleString("vi-VN", {
